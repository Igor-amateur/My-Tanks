...
namespace MyTanks
{
	...
	bool Battlefield::Show()
	{
		//Output the number of destroyed tanks
		std::cout << "\tDestroyed tanks = " << m_Destroyed;
	...
	void Battlefield::refresh(int & key)
	{
		...
		for (int hei = 0; hei < Height; hei++)
		{
			for (int wid = 0; wid < Width; wid++)
			{
				if (mp_battlefield[hei][wid] != nullptr)//exclude empty pointers We cut off the empty pointers
